...
class ZWaveModule(Module):
    ...
    def reset(self, request):
        try:
            ...
            returnStr = ZWaveService.reset(language=language)
    ...
    def network_add(self, request):
        try:
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.network_add(language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def network_remove(self, request):
        try:
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.network_remove(language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def network_abort(self, request):
        try:
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.network_abort(language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def network_list(self, request):
        try:
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.network_list(language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def network_update(self, request):
        try:
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.network_update(language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def network_learn(self, request):
        try:
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.network_learn(language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def basic_get(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.basic_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def basic_set(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.basic_set(node, value, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def door_lock_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.door_lock_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def door_lock_set(self, request):
        try:
            ...
            value = request.get_arg("value")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.door_lock_set(node, value, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def battery_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.battery_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def switch_multilevel_set(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.switch_multilevel_set(node, level, duration, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def switch_multilevel_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.switch_multilevel_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def switch_multilevel_start(self, request):
        try:
            ...
            returnStr = ZWaveService.switch_multilevel_start(node, primarySwitchDirection, primaryStartLevel, secondarySwitchDirection, secondarySwitchStep, duration, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def switch_multilevel_stop(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.switch_multilevel_stop(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def sensor_multilevel_get(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.sensor_multilevel_get(node, sensorType, unit, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def meter_capabilities_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.meter_capabilities_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def meter_get(self, request):
        try:
            ...
            unit = request.get_arg("unit")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.meter_get(node, unit, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def meter_reset(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.meter_reset(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def alarm_setup(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.alarm_setup(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def alarm_set(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.alarm_set(node, zwaveType, status, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def alarm_get(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.alarm_get(node, vendorType, zwaveType, zwaveEvent, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def alarm_supported_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.alarm_supported_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def alarm_supported_event_get(self, request):
        try:
            ...
            zwaveType = request.get_arg("zwaveType")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.alarm_supported_event_get(node, zwaveType, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def binary_sensor_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.binary_sensor_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def binary_switch_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.binary_switch_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def binary_switch_set(self, request):
        try:
            ...
            value = request.get_arg("value")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.binary_switch_set(node, value, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def switch_multilevel_supported_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.switch_multilevel_supported_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def sensor_multilevel_supported_get_sensor(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.sensor_multilevel_supported_get_sensor(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def wakeup_get(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.wakeup_get(node, scaleType, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def wakeup_set(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.wakeup_set(node, receiveNode, interval, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def configuration_set(self, request):
        try:
            ...
            value = request.get_arg("value")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.configuration_set(node, parameter, size, default, value, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def user_number_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.user_number_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def user_code_set(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.user_code_set(node, user, status, code, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def user_code_get(self, request):
        try:
            ...
            user = request.get_arg("user")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.user_code_get(node, user, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def association_groupings_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.association_groupings_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def association_specific_group_get(self, request):
        try:
            node = request.get_arg("node")
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.association_specific_group_get(node, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def association_get(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.association_get(node, group, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def association_set(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.association_set(node, group, setNode, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:
    ...
    def association_remove(self, request):
        try:
            ...
            language = request.get_arg(AppInfo.REQUEST_KEY_LANGUAGE)
            returnStr = ZWaveService.association_remove(node, group, removeNode, language=language)
            self.send_response(returnStr, request.requestId)
        except SystemException as se:
            self.send_response(se.value, request.requestId, se.value["returnValue"], se.value["returnMessage"])
        except Exception as e:

...
define([
    ...
    'common/components/views/component.control.option.select-all-item-renderer'
], function ($, _, AbstractControlComponent, JST, Model, kbxOptionItemRenderer, selectAllItemRenderer) {
    'use strict';

    var spr = AbstractControlComponent.prototype;
    var kbxOption = AbstractControlComponent.extend({
        ERROR_EMPTY_LIST: 'empty_list',
    	template: JST['app/scripts/common/components/templates/component.control.option.ejs'],
    	item_renderer: kbxOptionItemRenderer,
        select_all_item_renderer: selectAllItemRenderer,
    	destroy: function() {
    	...
    	render: function(model) {
            ...
            if (this.multiple && this.max === undefined || this.max >= this.model.get('list').length) {
                // if unlimited max or max is as same as list length
                // append select all first
                this.select_all = this.render_select_all();
                this.$('#list-container').append(this.select_all.el);
            ...
            this.model.get('list').each(function(item) {
                ...
                if (valueToBeSet != undefined && valueToBeSet.get(item.get('value')) !== undefined) {
            ...
            // invalidate select all item
            this.invalidate_select_all();

            this.trigger(this.EVT_INIT);
    	},

    	render_item: function(item_model) {
        ...
        render_select_all: function() {
            var itm = new this.select_all_item_renderer();
            itm.on('select', this.on_user_de_select_all, this);
            itm.render();
        ...
        on_user_de_select_all: function() {
            if (this.selected_item.length > 0) {
                ...
                // update selected item list
                this.selected_item = this.list_item.concat();
        ...
        invalidate_select_all: function() {
            if (this.select_all != undefined) {
                if (this.selected_item.length > 0) {
                    // select all
                    // allow deselect
                    this.select_all.select();

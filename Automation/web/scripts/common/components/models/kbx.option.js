...
define([
	...
	var kbxOption = kbxGeneric.extend({
		...
		clone: function() {
			var list;
			if (this.attributes.list) list = this.attributes.list.clone();
			var value;
			if (this.attributes.value) value = this.attributes.value.clone();
			var default_value;
			if (this.attributes.default_value) default_value = this.attributes.default_value.clone();
        ...
        isValueValid: function() {
            return this.get('value') != undefined && this.get('value').length > 0;
        },
		...
		LABEL_VALUE: kbxOptionListValueLabel
	})

    ...
    var CloudSession = KuroboxModel.extend({
        ...
        fetch: function(key, options) {
            ...
            if(success){
                ...
                //generate url
                this.dev_opt.param.url = Kurobox.cloudhost+'/c/client/login?'+this.objToParam(paramObj);
                //generate sum
        ...
        save: function(key, options) {
            ...
            if(success){
                ...
                //generate url
                this.dev_opt.param.url = Kurobox.cloudhost+'/c/client/registerUser?'+this.objToParam(paramObj);
        ...
        logout: function(username, options) {
            // user log out
            Kurobox.native('getLanguage', function(lang){
                this.dev_opt.param.url = Kurobox.cloudhost+'/c/client/logout;jsessionid='+this.session+'?sapi=1.0&uname='+username +'&lang='+lang;   
                this.dev_opt.param.concat = lang+'1.0'+username;
            ...
            if(success){
                Kurobox.native('getTemporaryDataOfKey', function(session){
                    ...
                    if(_.isEmpty(session)){
                        ...
                        //generate url
                        if(enable){
                            this.dev_opt.param.url = Kurobox.cloudhost+'/c/client/linkBox;jsessionid='+session+'?'+this.objToParam(paramObj);
                        }else{
                            this.dev_opt.param.url = Kurobox.cloudhost+'/c/client/unlinkBox;jsessionid='+session+'?'+this.objToParam(paramObj);
                        }
                        //generate sum
                        this.dev_opt.param.concate = this.concatParam(paramObj);

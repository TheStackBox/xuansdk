/**
* Copyright 2014-2015 Cloud Media Sdn. Bhd.
*
* This file is part of Xuan Automation Application.
*
* Xuan Automation Application is free software: you can redistribute it and/or modify
* it under the terms of the GNU General Public License as published by
* the Free Software Foundation, either version 3 of the License, or
* (at your option) any later version.
*
* Xuan Automation Application is distributed in the hope that it will be useful,
* but WITHOUT ANY WARRANTY; without even the implied warranty of
* MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
* GNU General Public License for more details.
*
* You should have received a copy of the GNU General Public License
* along with Xuan Automation Application.  If not, see <http://www.gnu.org/licenses/>.
*/
/*global define*/

define([
    'jquery',
    'underscore',
    'backbone',
    'templates',
    'models/rule.setting',
    'models/rule.device',
    'toggleswitch',
    'libs/utils/color',
    'farbtastic'
], function ($, _, Backbone, JST, RuleSettingTemplate, RuleDevice) {
    'use strict';

    var ColorRuleSetting = Backbone.View.extend({
    	template: JST['app/scripts/templates/module.rule.rule-setting.color.ejs'],
    	initialize: function() {
    		this.selected_color_hex = this.options.selected_color || '#ff00ff';
    		this.render();
    	},

    	render: function() {
    		this.$el.html(this.template());

    		_.defer(function() {
    			// initialize picker
	    		this.farbtastic = $.farbtastic(this.$('#picker'), this.on_user_pick_color.bind(this));
		        this.farbtastic.radius = 93;
		        this.farbtastic.square = 0;
		        this.farbtastic.width = 250;
		        this.farbtastic.setColor(this.selected_color_hex);

	            // disable inner square marker
	            // added a div to display color output
	            this.$('.color').before( '<div id="output"></div>' );
	            this.$('.sl-marker').css('display', 'none');
    		}.bind(this))
    	},

    	close: function() {
    		// change color
    		this.trigger('color_selected', this.selected_color_hex);

    		// destory everthing!
    		this.$el.remove();
    		this.undelegateEvents();
    	},

    	on_user_pick_color: function(code) {
            this.$("#output").css("background-color", code);

            this.selected_color_hex = code;
    	},
    })

    return ColorRuleSetting;
});